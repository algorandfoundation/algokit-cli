...
def test_localnet_reset_without_existing_sandbox(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_reset_with_existing_sandbox_with_out_of_date_config(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_reset_with_existing_sandbox_with_up_to_date_config(app_dir_mock: AppDirs, proc_mock: ProcMock):
    (app_dir_mock.app_config_dir / "sandbox").mkdir()
...
def test_localnet_reset_without_docker(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_reset_without_docker_compose(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_reset_without_docker_engine_running(app_dir_mock: AppDirs, proc_mock: ProcMock):
