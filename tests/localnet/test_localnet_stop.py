...
def test_localnet_stop(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_stop_failure(app_dir_mock: AppDirs, proc_mock: ProcMock):
    (app_dir_mock.app_config_dir / "sandbox").mkdir()
    (app_dir_mock.app_config_dir / "sandbox" / "docker-compose.yml").write_text("existing")
...
def test_localnet_stop_no_existing_definition(app_dir_mock: AppDirs, proc_mock: ProcMock):
    result = invoke("localnet stop")
...
def test_localnet_stop_without_docker(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_stop_without_docker_compose(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_localnet_stop_without_docker_engine_running(app_dir_mock: AppDirs, proc_mock: ProcMock):
