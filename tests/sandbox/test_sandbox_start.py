...
def test_sandbox_start_failure(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_up_to_date_definition(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_out_of_date_definition(app_dir_mock: AppDirs, proc_mock: ProcMock):
    (app_dir_mock.app_config_dir / "sandbox").mkdir()
...
def test_sandbox_start_without_docker(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_without_docker_compose(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_without_docker_engine_running(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_with_old_docker_compose_version(app_dir_mock: AppDirs, proc_mock: ProcMock):
...
def test_sandbox_start_with_unparseable_docker_compose_version(app_dir_mock: AppDirs, proc_mock: ProcMock):
