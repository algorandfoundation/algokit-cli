...
3. Design frontend templates such that backend and frontend have less than zero dependencies on each other, ensuring high modularity.
...
2. Implement a bundling process for backend and frontend templates in the starter repositories that code duplication and maintenance overhead.
...
- **Increased maintenance overhead**: The monorepo approach increases the maintenance overhead as it requires the templates to be updated together.
...
After several review rounds team reached a conclusion that this approach is not ideal for the following reasons:
...
After several review rounds team reached a conclusion to expand on this option given better alignment with AlgoKit design principles. Hence, the following proposal is further exploration of ideas based on this approach and propose solution to mitigate the potential drawbacks and risks outlined.
